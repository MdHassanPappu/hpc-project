...
--
## Industry Standard: OSU Micro-Benchmarks (OMB)
...
# Installation Methods: Technical Comparison

| Feature | Local Build | EESSI | EasyBuild |
|:--------|:----------:|:-----:|:---------:|
| **Performance** | ★★★★★ | ★★★★☆ | ★★★☆☆ |
| **Portability** | ★☆☆☆☆ | ★★★★★ | ★★★☆☆ |
| **Maintenance** | ★☆☆☆☆ | ★★★★☆ | ★★★★★ |
...
A **bit mask** is a binary pattern used to specify a set of CPUs for binding.  
- `1` means the CPU is included  
- `0` means the CPU is excluded 
-  the mask `0x2` (binary `10`) selects CPUs 1.
  
```{bash, echo=TRUE, eval=FALSE}
srun srun -N 1 -n 2 --cpu-bind=mask_cpu:0x1,0x10  osu_get_latency
```
...
```{bash, echo=TRUE, eval=FALSE}
srun srun -N 1 -n 2 --cpu-bind=mask_cpu:0x1,0x10000  osu_get_latency
```
...
```{bash, echo=TRUE, eval=FALSE}
srun -N 1 -n 2 --cpu-bind=mask_cpu:0x1,0x10000000000000000 osu_get_latency
```
- Task 0 on : CPU(s) 0, Socket 0, NUMA node 0
...
self.job.launcher.options = [    
    SRUN_OPTIONS,           
    './bind_same_numa.sh'  # Places both processes on same NUMA node
]
```
...
- Use task ID-based conditional binding
...
![Comparison of Bandwidth for different installation by Manually testing](imgs/Bandwidth_Manually.jpeg)

---
# Latency Comparison - Manual Testing

![Comparison of Latency for different installation by Manually testing](imgs/Latency_Manually.jpeg)
...
- **Local** installations generally achieved the highest bandwidth performance
- **Cross-node** communication shows expected higher latency but with varied bandwidth
- Several failed tests require investigation (marked in raw data)
]
---
...
.pull-right[
### Key Observations
- **Local installations** achieve highest bandwidth across most placement scenarios
- On AION, EESSI shows comparable performance to local builds
- Installation method impact varies significantly by cluster architecture
]
...
- Highest performance with **local build + cross-node** communication (~12,335 MB/s)
- Significant performance difference between installation methods
- NUMA-aware placement shows clear impact on communication efficiency
]
...
- Best performance with **local build + cross-node** communication (~8,625 MB/s)
- Lower overall bandwidth than AION across all scenarios
- Smaller performance gap between installation methods compared to AION
]
...
- Radar visualization reveals installation method strengths across placement scenarios
- AION shows larger "footprint" indicating better overall bandwidth performance
- Installation methods show unique performance profiles across placement types
]
...
- Consistent bandwidth hierarchy: Local > EESSI > EasyBuild
- AION shows 1.4-2.5× bandwidth advantage over IRIS across most scenarios
- Placement impact is consistent across both clusters
- Some outlier results warrant further investigation
]
...
.pull-right[
### Latency Insights
- All installation methods achieve similar latency within each placement scenario
- **Cross-node communication** shows expected higher latency (~3.8μs on AION, ~5.3μs on IRIS)
- AION consistently delivers lower latency across all test cases
]
...
- Extremely consistent intra-node latency (~0.2μs) regardless of installation method
- Cross-node latency (~3.8μs) shows minimal variation between installation methods
- Installation method has minimal impact on latency performance
]
...
- Higher latency values across all placement scenarios compared to AION
- Same-NUMA latency (~0.7μs) is notably higher than on AION
- Cross-node latency (~5.3μs) represents significant communication overhead
]
...
- Smaller radar footprint indicates better latency performance (lower values)
- AION shows clear latency advantage across all dimensions
- Installation methods show minimal impact on latency performance profile
]
...
- Architecture matters more than installation method for latency performance
- AION achieves ~3.5× better same-NUMA latency compared to IRIS
- Cross-node communication shows expected higher latency on both clusters
]
...
- **Installation Impact**: Local builds consistently deliver best performance, especially for bandwidth
- **Architectural Differences**: AION significantly outperforms IRIS across all test cases
  - Bandwidth: Up to 2.5× higher on AION
  - Latency: Up to 3.5× lower on AION
  
## Practical Implications
- **Application-specific considerations**:
  - Bandwidth-sensitive applications benefit most from local builds
  - Latency-sensitive applications see minimal benefit from installation choice
  - Process placement remains critical regardless of installation method
  
## Recommendations
- Use **locally-built MPI** for bandwidth-critical applications
- Prioritize **proper process placement** for both bandwidth and latency optimization
- For portable software stacks, **EESSI** offers good performance/portability balance
